...
const EditProfile = () => {
  let { userName, changeUserName, userEmail, setUserEmail, userPassword } = useContext(UserContext);
  ...
  const sendOtp = async () => {
    try {
      setOtpSent(true);
      const response = await axios.post('/api/users/send-otp', { email: newEmail });
  ...
  const verifyOtp = async () => {
    ...
    try {
      const response = await axios.post('/api/users/verify-otp', { email: newEmail, otp: otpString });
      if (response.status === 200) {
        setOtpVerified(true);
              ...
              <Form.Group controlId="formEmail" disabled>
                ...
                <InputGroup className="mb-3">
                  <Form.Control
                    ...
                    disabled
                  />
                  <Button variant="outline-secondary" onClick={handleShowEditEmail}>
                    Change Email-id
                  </Button>
                </InputGroup>
              </Form.Group>
                  ...
                  {otpReceived && (
                    <div>
                      <Form.Group controlId="formOtp">
                        ...
                        <Button variant="primary" className="mt-3" onClick={verifyOtp}>
                          Verify OTP
                        </Button>
                      </Form.Group>
                    </div>
